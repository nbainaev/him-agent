...
class ApicalTiebreakTemporalMemory(object):
    ...
    def __init__(
        ...
        self.basalConnections = Connections(
            columnCount * cellsPerColumn, connectedPermanence, False
        )
        self.apicalConnections = Connections(
            columnCount * cellsPerColumn, connectedPermanence, False
        )
    ...
    def depolarizeCells(self, basalInput, apicalInput, learn):
        ...
        ) = self._calculateApicalSegmentActivity(apicalInput)
        ...
        ) = self._calculateBasalSegmentActivity(basalInput, reducedBasalThresholdCells)
    ...
    def activateCells(
        ...
        correctPredictedCells, burstingColumns = np2.setCompare(
            self.predictedCells, activeColumns, self.predictedCells // self.cellsPerColumn,
            rightMinusLeft=True
        )
        newActiveCells = np.concatenate(
            (correctPredictedCells, np2.getAllCellsInColumns(burstingColumns, self.cellsPerColumn))
        )
        ...
        (
            ...
            learningCells
        ) = self._calculateBasalLearning(activeColumns, burstingColumns, correctPredictedCells)
        ...
        ) = self._calculateApicalLearning(learningCells, activeColumns)
        ...
        if learn:
            ...
            for learningSegments in (learningActiveBasalSegments, learningMatchingBasalSegments):
                self._learn(
                    self.basalConnections, learningSegments, basalReinforceCandidates,
                    basalGrowthCandidates, self.basalPotentialOverlaps
                )

            for learningSegments in (learningActiveApicalSegments, learningMatchingApicalSegments):
                self._learn(
                    self.apicalConnections, learningSegments, apicalReinforceCandidates,
                    apicalGrowthCandidates, self.apicalPotentialOverlaps
                )

            # Punish incorrect predictions
            if self.basalPredictedSegmentDecrement != 0.0:
                for segment in basalSegmentsToPunish:
                    self.basalConnections.adaptSegment(
                        segment, basalReinforceCandidates, -self.basalPredictedSegmentDecrement,
                        0.0, False
                    )

            if self.apicalPredictedSegmentDecrement != 0.0:
                for segment in apicalSegmentsToPunish:
                    self.apicalConnections.adaptSegment(
    ...
    def _calculateBasalLearning(self, activeColumns, burstingColumns, correctPredictedCells):
        ...
        learningActiveBasalSegments = self.basalConnections.filterSegmentsByCell(
            self.activeBasalSegments, correctPredictedCells
        )
        ...
        matchingCellsInBurstingColumns, burstingColumnsWithNoMatch = np2.setCompare(
        ...
        learningMatchingBasalSegments = self._chooseBestSegmentPerColumn(
            self.basalConnections, matchingCellsInBurstingColumns, self.matchingBasalSegments,
            self.basalPotentialOverlaps
        )
        newBasalSegmentCells = self._getCellsWithFewestSegments(
            self.basalConnections, burstingColumnsWithNoMatch
        )
        ...
        correctMatchingBasalMask = np.isin(
    ...
    def _calculateApicalLearning(self, learningCells, activeColumns):
        ...
        learningActiveApicalSegments = self.apicalConnections.filterSegmentsByCell(
            self.activeApicalSegments, learningCells
        )

        # Cells with matching apical segments
        learningCellsWithoutActiveApical = np.setdiff1d(
            learningCells, self.apicalConnections.mapSegmentsToCells(learningActiveApicalSegments)
        )
        ...
        learningMatchingApicalSegments = self._chooseBestSegmentPerCell(
            self.apicalConnections, learningCellsWithMatchingApical, self.matchingApicalSegments,
            self.apicalPotentialOverlaps
        )

        # Cells that need to grow an apical segment
        newApicalSegmentCells = np.setdiff1d(
            learningCellsWithoutActiveApical, learningCellsWithMatchingApical
        )

        # Incorrectly predicted columns
        correctMatchingApicalMask = np.isin(
    ...
    def _calculateBasalSegmentActivity(self, activeInput, reducedBasalThresholdCells):
        ...
        if self.reducedBasalThreshold != self.activationThreshold and len(
                reducedBasalThresholdCells
        ) > 0:
            potentiallyActiveSegments = np.flatnonzero(
                (overlaps < self.activationThreshold) & (overlaps >= self.reducedBasalThreshold)
            )
            ...
            conditionallyActiveSegments = potentiallyActiveSegments[
                np.isin(cellsOfCASegments, reducedBasalThresholdCells)]
    ...
    def _calculatePredictedCells(self, activeBasalSegments, activeApicalSegments):
        ...
        if not self.useApicalTiebreak:
            ...
            inhibitedMask = np.isin(
                partlyDepolarizedCells // self.cellsPerColumn,
                fullyDepolarizedCells // self.cellsPerColumn
            )
            predictedCells = np.append(
                fullyDepolarizedCells, partlyDepolarizedCells[~inhibitedMask]
            )

        return predictedCells

    def _learn(
            self, connections, learningSegments, activeInput, growthCandidates, potentialOverlaps
    ):
        """
        Adjust synapses permanence, grow new synapses, and grow new segments.
        ...
        for segment in learningSegments:
            connections.adaptSegment(
                segment, activeInput, self.permanenceIncrement, self.permanenceDecrement, False
            )
            ...
            if maxNew > 0:
                connections.growSynapses(
                    segment, growthCandidates, self.initialPermanence, self.rng, maxNew
                )
    ...
    def _learnOnNewSegments(self, connections, newSegmentCells, growthCandidates):
        ...
        for cell in newSegmentCells:
            ...
            connections.growSynapses(
                newSegment, growthCandidates, self.initialPermanence, self.rng,
                maxNew=numNewSynapses
            )

    @staticmethod
    def _chooseBestSegmentPerCell(
            connections,
            cells,
            allMatchingSegments,
            potentialOverlaps
        ...
        onePerCellFilter = np2.argmaxMulti(
            potentialOverlaps[candidateSegments], connections.mapSegmentsToCells(candidateSegments)
        )
    ...
    def _chooseBestSegmentPerColumn(
            self, connections, matchingCells, allMatchingSegments, potentialOverlaps
    ):
        """
        ...
        columnsForCandidates = (
                    connections.mapSegmentsToCells(candidateSegments) // self.cellsPerColumn
        )
    ...
    def _getCellsWithFewestSegments(self, connections, columns):
        ...
        segmentCounts = np.reshape(
            ...
            newshape=(len(columns), self.cellsPerColumn)
        )
        ...
        _, onePerColumnFilter, numCandidatesInColumns = np.unique(
            candidateCells // self.cellsPerColumn, return_index=True, return_counts=True
        )
        ...
        np.add(
            onePerColumnFilter, offsetPercents * numCandidatesInColumns, out=onePerColumnFilter,
            casting="unsafe"
        )
    ...
    def setBasalPredictedSegmentDecrement(self, basalPredictedSegmentDecrement):
        ...
        @param basalPredictedSegmentDecrement (float) The predicted segment decrement.
        """
        self.basalPredictedSegmentDecrement = basalPredictedSegmentDecrement
    ...
    def setApicalPredictedSegmentDecrement(self, apicalPredictedSegmentDecrement):
        """
        Sets the predicted segment decrement.
        @param apicalPredictedSegmentDecrement (float) The predicted segment decrement.
        """
        self.apicalPredictedSegmentDecrement = apicalPredictedSegmentDecrement
...
class ApicalTiebreakPairMemory(ApicalTiebreakTemporalMemory):
    ...
    def compute(
        ...
        basalInputSDR.sparse = basalInput
        self.activateCells(
            activeColumns, basalInputSDR, apicalInputSDR, basalGrowthCandidates,
...
class ApicalTiebreakSequenceMemory(ApicalTiebreakTemporalMemory):
    ...
    def compute(
        ...
        self.activateCells(
            activeColumns, activeCellsSDR, apicalInputSDR, self.winnerCells,
            self.prevApicalGrowthCandidates, learn
        )
