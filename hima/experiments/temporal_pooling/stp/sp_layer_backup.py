...
class SpatialPooler:
    ...
    def __init__(
        ...
        self.i_presynaptic = self.rf.flatten()
        self.i_postsynaptic = np.arange(self.output_size).repeat(rf_size)

        self.rf_sizes = np.full(self.output_size, rf_size)

        print(f'SP.layer init shape: {self.output_size} x {rf_size} = {len(self.i_presynaptic)}')
        self.hidden_weight_masses = self.rng.normal(
            loc=0.5, scale=initial_weight_noise, size=self.output_size
        )
        self.weights = normalize_weights(
            self.rng.normal(loc=1.0, scale=initial_weight_noise, size=self.rf.shape)
        ).flatten()
        self.rf = self.rf.tolist()
    ...
    def _compute(self, input_sdr: AnySparseSdr, learn: bool) -> AnySparseSdr:
        ...
        self.potentials[self.i_postsynaptic] += matched_input_activity * self.weights
    ...
    def match_current_input(self, with_neurons: np.ndarray = None):
        if with_neurons is None:
            i_presynaptic = self.i_presynaptic
        else:
            assert False, 'Not implemented'

        return self.dense_input[i_presynaptic]
    ...
    def reinforce_winners(self, matched_input_activity, learn: bool):
        if not learn:
            return
        self.stdp(self.winners, matched_input_activity)
    ...
    def _stdp_new(
        ...
        for i_win in neurons:
            ...
            w = self.weights[i_synapses]
            dw = lr * post_rates * (pre_rates - post_rates * w)

            self.weights[i_synapses] = ip_1d_normalize_weights(w + dw)
...
def ip_1d_normalize_weights(weights):
    normalizer = np.abs(weights).sum()
    return np.clip(weights / normalizer, 0., 1.)
