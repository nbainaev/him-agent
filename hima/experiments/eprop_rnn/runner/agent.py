...
import numpy as np
from hima.experiments.tpcn.runner.encoders import SimpleOneHotEncoder
...
class RNNWithEPropWrapper(BaseAgent):
    ...
    def observe(self, obs, action, reward=0):
        return self.agent.observe(self.obs_encoder.transform(obs), self.action_encoder.transform(action), reward)
