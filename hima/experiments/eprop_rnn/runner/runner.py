...
class TPCNRunner(BaseRunner):
    ...
    def make_agent(agent_type, conf):
        if agent_type == 'eprop_rnn':
            from hima.experiments.eprop_rnn.runner.agent import RNNWithEPropWrapper
    ...
    def save_model(self, dir_path):
        ...
        if isinstance(model, RNNWithEProp):
            pkl.dump(model, os.path.join(dir_path, f'{self.logger.name}_{self.episodes}.pkl'))
