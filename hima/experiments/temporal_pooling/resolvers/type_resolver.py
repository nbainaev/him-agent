...
def _resolve_spatial_pooler(type_tag: str):
    ...
    if type_tag == 'sp.layer_mc':
        from hima.experiments.temporal_pooling.stp.sp_layer_multicomp import SpatialPooler
        return SpatialPooler
    ...
    if type_tag == 'sp.soft_hebb_ext':
        from hima.experiments.temporal_pooling.stp.soft_hebb_ext import SoftHebbLayer
        return SoftHebbLayer
    if type_tag == 'sp.krotov':
        from hima.experiments.temporal_pooling.stp.krotov import KrotovLayer
        return KrotovLayer
    if type_tag == 'sp.krotov_ext':
        from hima.experiments.temporal_pooling.stp.krotov_ext import KrotovLayer
        return KrotovLayer
    if type_tag == 'sp.se':
...
def _resolve_decoder(type_tag):
    ...
    if type_tag == 'decoder.mlp_torch':
        from hima.experiments.temporal_pooling.stp.mlp_decoder_torch import MlpDecoder
        return MlpDecoder
