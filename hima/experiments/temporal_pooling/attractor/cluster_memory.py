...
class ClusterMemory:
    ...
    def __init__(
            self, sds: Sds, similarity_threshold: TConfig,
            max_n_clusters: int, max_tracked_bits_rate: float = 3.,
    ):
        self.sds = sds
        self.similarity_threshold = BalancingParam(**similarity_threshold)

        max_n_tracked_bits = int(self.n_active_bits * max_tracked_bits_rate)
        self.n_clusters = 0
        self._clusters = np.zeros((max_n_clusters, max_n_tracked_bits), dtype=int)
        self._clusters_sets = []
        self._density = np.zeros_like(self._clusters, dtype=float)

        self._cluster_sizes = np.zeros(max_n_clusters, dtype=int)
        self._cluster_traces = np.zeros(max_n_clusters, dtype=float)

        self._cache_sdr = np.zeros(self.sds.size, dtype=int)
        self.stats = ClusterMemoryStats()

    @property
    def n_active_bits(self):
        return self.sds.active_size
    ...
    @property
    def clusters(self):
        """Gets indices of clusters' elements (with non-tracked)."""
        return self._clusters[:self.n_clusters]

    @property
    def density(self):
        """Gets density of clusters' elements (with non-tracked)."""
        return self._density[:self.n_clusters]

    def select_tracked(self, arr, ind):
        """Select only tracked elements from arr for cluster ind."""
        return arr[-self._cluster_sizes[ind]:]
    ...
    @property
    def traces(self):
        return self._cluster_traces[:self.n_clusters]
    ...
    def create(self, sdr: SparseSdr) -> int:
        ...
        self._clusters_sets.append(sdr_set)
    ...
    def update(self, cluster: int, sdr: SparseSdr):
        ...
        if max_cluster_size < size + k:
            ...
            cleared_set = set(cleared[cleared_density > 1e-10])
